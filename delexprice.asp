...
if straction="delete" then
	...
	if strid<>"" then
		strsql="delete from exportprice where id in ("& strid & ")"
		conn.execute strsql
	end if
	response.redirect "delexprice.asp"
end if
%>
...
function dodelcategory()
{
	...
	if (document.delcategory.chkid!=null)
	{
		...
		if(i>0 && k==0)
		strmsg="You must select one exportprice to delete!";
		if(i==0)
		{
  ...
  <tr>
    <td width="644" valign="top">
      <div align="center"><a href="DatAdmin.asp">Back 
        ...
        <p>Delete Exportprice</p>
        <form name="delcategory" action="delexprice.asp" method="post">
          <table width="66%" border="0" cellspacing="1" cellpadding="4" bgcolor="#C0C0C0">
            ...
            <tr bgcolor="#336699"> 
              ...
              <td colspan="3" bgcolor="#FFFFFF"><%= trim(acres("exportprice")&"") %>&nbsp;</td>
            </tr>
            <%	
			  		acres.movenext
					j=j+1
...
function showpageno(pageno)
	if recount>10 then
		...
		if pageno>10 then
		     response.write "<a href='delexprice.asp?pageno=1'> The First Page</a>&nbsp;&nbsp;"
			response.write "<a href='delexprice.asp?pageno="&(pageno-9-(pageno  mod 10) )&"'>Previous 10</a>&nbsp;&nbsp;"
		end if
		strtemp=pageno
	 ...
	 for i=(strtemp-(strtemp mod 10)+1) to (strtemp+10-(strtemp mod 10))
            ...
            if i- pageno=0 then
				...
				response.write "<a href='delexprice.asp?Pageno="&i&"'>  "&cstr(i)&" </a>&nbsp;&nbsp;"
			end if	
		next
		if (pageno\10)<(lastpage\10) then
		        response.write "<a href='delexprice.asp?Pageno="&(pageno+11-(pageno mod 10)) &"'>Next 10</a>&nbsp;&nbsp;"
			   response.write "<a href='delexprice.asp?Pageno="& (lastpage) &"'> The Last Page</a>&nbsp;&nbsp;"
		end if
