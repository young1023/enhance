...
<%
if session(("SECURITY_LEVEL"))<>"1" or session("product_id")="" then
  response.redirect "gologin.asp"
end if

strprodid=session("product_id")
picflage=session("picflage")
'response.write picflage
'response.end
if trim(picflage)="" then
  picflage=0
end if
set rec=server.createobject("ADODB.recordset")
rec.Open "SELECT picpath,pic2path FROM product where id=" & strprodid & " ",conn,1,3

if not rec.eof then
  if picflage=1 then
    rec("picpath")=null
    rec.update
  elseif picflage=2 then
    rec("pic2path")=null
    rec.update
  end if
end if
...
'session("product_id")=""
if picflage=1 then
  StrMsg="The small picture was deleted!"
elseif picflage=2 then
  StrMsg="The big picture was deleted!"
else
  StrMsg="<font color=red>The picture couldn't be deleted!</font>"
end if
%>
<HTML><HEAD>
