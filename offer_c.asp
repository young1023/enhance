...
strsearchsql=""
	...
	strsearchsql="SELECT product.description, product.offer, product.Model, exportprice.Exportprice, category.Category, " _
    ...
    <tr> 
      <td valign="top" width="790" align="center"> 
            ...
            <% 
				...
				<% if recount=1 then%>           
				  <br><font size="1"> 
           找到 <%=recount%> 個特別推介.</font> 
            <%else%>		                   
				<br><font size="1"> 找到 <%=recount%> 個特別推介.</font> 
		   ...
		   <form name="form1" action="enquiry_c.asp" method="post">
            <table width="80%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
              ...
              <tr> 
				  ...
				  %>
                       <a href="offer_c.asp?action_button=<%= server.URLencode(straction) %>&strwhere=<%= server.URLencode(strwhere) %>&pageno=<%= 1 %>"> 
...
<table width="80%" border="0" cellspacing="5" cellpadding="2" bgcolor="#FFFFFF">
         <tr > 
 ...
 <a href="productdetail_c.asp?id=<%=trim(acres("id")&"") %>"> 
                  ...
                  <img src="showimg.asp?id=<%=acres("id")%>&flage=2" border=1 width="200" height="150">
                  <%else  
...
<%				    loop
              ...
              <tr > 
                <td colspan="10"> 
                      <div align="center"> 
                        <font size="2"> 
                        <input type="button" value="加入報價表" onClick="javascript:dosubmit();" name="button">
                        <%  
...
<table width="98%" border="0" cellspacing="5" height=200 cellpadding="2" bgcolor="#FFFFFF">
         <tr > 
            ...
            <font size="2" color="#000000">沒有紀錄</font>.
              </td>
         <tr>
</table>
            <% End If %>                                     
...
function showpageno(pageno,straction,strwhere) 
	if recount>9 then 
		...
		if pageno>9 then 
		     response.write "<a href='offer_c.asp?action_button="&server.URlencode(straction)&"&strwhere="& server.URLencode(strwhere)&"&pageno=1'> The First Page</a>&nbsp;&nbsp;" 
			response.write "<a href='offer_c.asp?action_button="&server.URlencode(straction)&"&strwhere="& server.URLencode(strwhere)&"&pageno="&(pageno-9-(pageno  mod 9) )&"'>Previous </a>&nbsp;&nbsp;" 
		end if 
		strtemp=pageno 
	 ...
	 for i=(strtemp-(strtemp mod 9)+1) to (strtemp+9-(strtemp mod 9)) 
            ...
            if i- pageno=0 then 
				...
				response.write "<a href='offer_c.asp?action_button="&server.URlencode(straction)&"&strwhere="& server.URLencode(strwhere)&"&Pageno="&i&"'>  "&cstr(i)&" </a>&nbsp;&nbsp;" 
			end if	 
		next 
		if (pageno\9)<(lastpage\9) then 
		       	response.write "<a href='offer_c.asp?action_button="&server.URlencode(straction)&"&strwhere="& server.URLencode(strwhere)&"&Pageno="&(pageno+11-(pageno mod 9)) &"'>Next</a>&nbsp;&nbsp;" 
			   response.write "<a href='offer_c.asp?action_button="&server.URlencode(straction)&"&strwhere="& server.URLencode(strwhere)&"&Pageno="& (lastpage) &"'> The Last Page</a>&nbsp;&nbsp;" 
		end if 
