...
if straction="getpwd" then
  email=replace(trim(request("txtfemail")),"'","''")
  strsql="select question from member where email='"& email &"' "
  set acre=conn.execute(strsql)
  if not acre.eof then
    set acre=nothing
    set strsql=nothing
    response.redirect "forgetpwd2.asp?email="+email
  else
    strpwd="Sorry,Could not find the E-mail!<br><br>For further assistance,send us mail <a href='mailto:info@jaguarpen.com'>info@jaguarpen.com</a>"
  end if
  set acre=nothing
  set strsql=nothing
end if
...
{
	var errmsg="";
	if(document.frmpwd.txtfemail.value=="")
		errmsg=errmsg+"Plesase Input Login Email!\n";
	if (errmsg!="")
		alert(errmsg);
	else{
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" height="455">
  ...
  <tr>
    <td width="644" valign="top">
      <div align="center">
	  <% If strpwd="" then %>
	    <form name="frmpwd" method="post" action="forgetpwd.asp">
          <table width="60%" border="1" cellspacing="1" cellpadding="0">
            <tr> 
              <td><b>Login Email :</b></td>
              <td><b> 
                <input type="text" name="txtfemail" size="30" maxlength="50">
                </b></td>
            </tr>
            <tr> 
              <td colspan="2"> 
      ...
      <font size="2" face="Arial, Helvetica, sans-serif" color="#FFFFFF"> 
        ...
        <%=strpwd %></font><font size="2" face="Arial, Helvetica, sans-serif" color="#FFFFFF"> 
  </font></div>
